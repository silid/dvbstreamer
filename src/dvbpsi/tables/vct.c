                              ...
                              dvbpsi_psi_section_t * p_section)
{
  ...
  if(b_reinit)
  {
    ...
    for(i = 0; i <= 255; i++)
    {
      ...
      {
        dvbpsi_ReleasePSISections(p_psi_decoder, p_vct_decoder->ap_sections[i]);
        p_vct_decoder->ap_sections[i] = NULL;
      }
    }
  }
  ...
  if(b_append)
  {
    ...
    if(p_vct_decoder->ap_sections[p_section->i_number] != NULL)
    {
      DVBPSI_DEBUG_ARG("VCT decoder", "overwrite section number %d",
                       p_section->i_number);
      dvbpsi_ReleasePSISections(p_psi_decoder, p_vct_decoder->ap_sections[p_section->i_number]);
    }
