...
typedef struct dvbpsi_stt_decoder_s
{
  dvbpsi_stt_callback           pf_callback;
  void *                        p_cb_data;

} dvbpsi_stt_decoder_t;

dvbpsi_descriptor_t *dvbpsi_STTAddDescriptor(
                                               dvbpsi_stt_t *p_stt,
                                               uint8_t i_tag, uint8_t i_length,
                                               uint8_t *p_data);

void dvbpsi_GatherSTTSections(dvbpsi_decoder_t * p_psi_decoder,
                              void * p_private_decoder,
                              dvbpsi_psi_section_t * p_section);

void dvbpsi_DecodeSTTSections(dvbpsi_stt_t* p_stt,
                              dvbpsi_psi_section_t* p_section);
                          ...
                          dvbpsi_stt_callback pf_callback, void* p_cb_data)
{
  ...
  dvbpsi_stt_decoder_t*  p_stt_decoder;
  ...
  p_stt_decoder = (dvbpsi_stt_decoder_t*)malloc(sizeof(dvbpsi_stt_decoder_t));
  ...
  p_subdec->pf_callback = &dvbpsi_GatherSTTSections;
  p_subdec->p_cb_data = p_stt_decoder;
  p_subdec->i_id = (uint32_t)i_table_id << 16;
  p_subdec->pf_detach = dvbpsi_DetachSTT;
 ...
 * Close a STT decoder.
 *****************************************************************************/
void dvbpsi_DetachSTT(dvbpsi_demux_t * p_demux, uint8_t i_table_id, uint16_t i_extension)
{
  dvbpsi_demux_subdec_t* p_subdec;
  ...
  p_stt_decoder = (dvbpsi_stt_decoder_t*)p_subdec->p_cb_data;
 ...
 * dvbpsi_InitSTT
 *****************************************************************************
 * Initialize a pre-allocated dvbpsi_stt_t structure.
 *****************************************************************************/
 ...
 * dvbpsi_EmptySTT
 *****************************************************************************
 * Clean a dvbpsi_stt_t structure.
 *****************************************************************************/
...
{
  dvbpsi_stt_decoder_t * p_stt_decoder
                        = (dvbpsi_stt_decoder_t*)p_private_decoder;
  ...
  {
      dvbpsi_NewSTT(p_stt, p_section->p_payload_start[0]);
      if (p_stt)
      {
          /* Decode the sections */
          dvbpsi_DecodeSTTSections(p_stt,
                                  p_section);
          /* Delete the sections */
                              ...
                              dvbpsi_psi_section_t* p_section)
{
    ...
    while(p_byte + 2 <= p_end)
    {
        ...
        if(i_length + 2 <= p_end - p_byte)
          dvbpsi_STTAddDescriptor(p_stt, i_tag, i_length, p_byte + 2);
        p_byte += 2 + i_length;
    }
