 ...
 *****************************************************************************/
/*!
 * \struct dvbpsi_mgt_table_s
 * \brief MGT Table structure.
 *
 * This structure is used to store a decoded MGT table information.
 */
/*!
 * \typedef struct dvbpsi_mgt_table_s dvbpsi_mgt_table_t
 * \brief dvbpsi_mgt_table_t type definition.
 */
typedef struct dvbpsi_mgt_table_s
{
    ...
    struct dvbpsi_mgt_table_s *p_next;      /*!< next element of the list */
} dvbpsi_mgt_table_t;
 ...
 */
typedef struct dvbpsi_mgt_s
{
    uint8_t                 i_version;          /*!< version_number */
    int                     b_current_next;     /*!< current_next_indicator */    
    uint8_t                 i_protocol;         /*!< PSIP Protocol version */

    dvbpsi_mgt_table_t     *p_first_table;      /*!< First table information structure. */
...
/*****************************************************************************
 * dvbpsi_InitMGT/dvbpsi_NewMGT
 *****************************************************************************/
/*!
 * \fn void dvbpsi_InitMGT(dvbpsi_mgt_t* p_mgt, uint8_t i_version, 
 ...
 * \return nothing.
 */
void dvbpsi_InitMGT(dvbpsi_mgt_t *p_mgt, uint8_t i_version, 
    int b_current_next, uint8_t i_protocol);

/*!
 * \def dvbpsi_NewMGT(p_mgt, i_network_id, i_version, b_current_next)
 ...
 * \return nothing.
 */
#define dvbpsi_NewMGT(p_mgt, i_version, b_current_next, i_protocol) \
do {                                                                    \
  p_mgt = (dvbpsi_mgt_t*)malloc(sizeof(dvbpsi_mgt_t));                  \
  if(p_mgt != NULL)                                                     \
    dvbpsi_InitMGT(p_mgt, i_version, b_current_next, i_protocol); \
} while(0);
...
/*!
 * \def dvbpsi_DeleteMGT(p_mgt)
 * \brief Clean and free a dvbpsi_mgt_t structure.
 * \param p_mgt pointer to the MGT structure
 * \return nothing.
 */
#define dvbpsi_DeleteMGT(p_mgt)                                         \
do {                                                                    \
  dvbpsi_EmptyMGT(p_mgt);                                               \
  free(p_mgt);                                                          \
} while(0);
